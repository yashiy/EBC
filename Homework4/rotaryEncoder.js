#!/usr/bin/env node
// This uses the eQEP hardware to read a rotary encoder
// echo bone_eqep2b > $SLOTS
var b = require('bonescript'),
    fs = require('fs');
var eQEP0 = "/sys/devices/ocp.3/48300000.epwmss/48300180.eqep/",
    eQEP1 = "/sys/devices/ocp.3/48302000.epwmss/48302180.eqep/",
    eQEP2 = "/sys/devices/ocp.3/48304000.epwmss/48304180.eqep/";

//eQEP = eQEP2;
var oldData;            // pervious data read
var period = 100;       // in ms
// Set the eEQP period, convert to ns.
 fs.writeFile(eQEP1+'period', period*1000000, function(err) {
        if (err) throw err;
        console.log('Period1 updated to ' + period*1000000);
 })

 fs.writeFile(eQEP2+'period', period*1000000, function(err) {
        if (err) throw err;
        console.log('Period2 updated to ' + period*1000000);
 })



 // Enable
 fs.writeFile(eQEP1+'enabled', 1, function(err) {
     if (err) throw err;
        console.log('Enabled1');
 })

 fs.writeFile(eQEP2+'enabled', 1, function(err) {
     if (err) throw err;
        console.log('Enabled2');
 })


setInterval(readEncoder, period);    // Check state every 250 ms
function readEncoder(x) {
    fs.readFile(eQEP1 + 'position', {encoding: 'utf8'}, upDown);
    fs.readFile(eQEP2 + 'position', {encoding: 'utf8'}, leftRight);	 
}


 function upDown(err, data) {
     if (err) throw err;
     if (oldData !== data) {
         //console.log('Position1: '+data+' speed1: '+(oldData-data));
         oldData = data;
         

     }
 }

 function leftRight(err, data) {
     if (err) throw err;
     if (oldData !== data) {
         console.log('Position2: '+data+' speed2: '+(oldData-data));
         oldData = data;
         }
 }

